...
func main() {
	...
	http.Handle("/", home(hlivekit.NewCacheRistretto(cache)))
...
func home(cache l.Cache) *l.PageServer {
	f := func() *l.Page {
		page := l.NewPage(l.PageOptionCache(cache))
		page.SetLogger(zerolog.New(zerolog.ConsoleWriter{Out: os.Stderr, TimeFormat: "15:04:05"}).With().Timestamp().Logger().
...
func newTodoApp() *todoApp {
	...
	//loops := rand.Intn(5-1) + 1
	//for i := 0; i < loops; i++ {
	//	app.addTask(strconv.Itoa(rand.Int()))
	//}
